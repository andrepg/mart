...
void main() async {
  ...
  test('save single grocery list', () async {
    var groceryList = GroceryList(title: "Fruits");

    await GroceryListPersistence.instance.saveSingle(groceryList.toMap());
  ...
  test('save collection of grocery lists', () async {
    var groceries = [
  ...
  test('fetch single list', () async {
    await db.insert("grocery_lists", GroceryList(title: "Fruits").toMap());

    var groceryList = await GroceryListPersistence.instance.loadSingle('1');
    expect(groceryList[0]['id'], isNotNull);
    expect(groceryList[0]['title'], equals("Fruits"));
  });

  test('fetch collection from database', () async {
